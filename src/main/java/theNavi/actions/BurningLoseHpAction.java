...
public class BurningLoseHpAction extends AbstractGameAction {
    ...
    public BurningLoseHpAction(AbstractCreature target, AbstractCreature source, int amount, AttackEffect effect) {
        ...
        if (AbstractDungeon.getCurrRoom().phase != RoomPhase.COMBAT) {
            ...
            if (this.isDone) {
                if (this.target.currentHealth > 0) {
                    this.target.tint.color = Color.CHARTREUSE.cpy();  //TODO: Change color to red?
                    this.target.tint.changeColor(Color.WHITE.cpy());
                    this.target.damage(new DamageInfo(this.source, this.amount, DamageType.HP_LOSS));
                }
//                This is used to downtick poison
